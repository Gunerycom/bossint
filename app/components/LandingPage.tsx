...
export default function LandingPage({ onLogin }: LandingPageProps) {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    if (!email) {
      setError("Please enter your email or API key.");
      return;
    }

    // Direct API Key Login Support
    if (email.trim().startsWith("sk_")) {
      setIsLoading(true);
      localStorage.setItem("bossint_auth", "true");
      localStorage.setItem("bossint_user_token", email.trim());
      localStorage.setItem("bossint_user_email", "saas-user@bossint.ai");
    ...
    if (success) {
      ...
      // Fallback for offline/unregistered dev mode (uses the supplied SaaS API key)
      if (password === "112233q" && (email.trim().toLowerCase() === "gokhan@bossint.ai" || email.trim().toLowerCase() === "gokhan@gunery.com")) {
        localStorage.setItem("bossint_auth", "true");
        localStorage.setItem("bossint_user_token", "sk_1f6a88dd6e054a399e971163a1e15c6b");
        localStorage.setItem("bossint_user_email", "gokhan@bossint.ai");
        window.dispatchEvent(new Event("bossint_auth_change"));
        onLogin();
      } else {
  ...
  const handleQuickDemoLogin = async () => {
    setError("");
    setIsLoading(true);
    const success = await performLogin("gokhan@bossint.ai", "112233q");
    if (success) {
      window.dispatchEvent(new Event("bossint_auth_change"));
      onLogin();
    } else {
  ...
  const handleModalAuthSubmit = async (e: React.FormEvent) => {
    ...
    setModalError("");
    if (!modalEmail) {
      setModalError("Please enter your email or API key.");
      return;
    }

    // Direct API Key Login Support for Modal
    if (modalEmail.trim().startsWith("sk_")) {
      setIsLoading(true);
      localStorage.setItem("bossint_auth", "true");
      localStorage.setItem("bossint_user_token", modalEmail.trim());
      localStorage.setItem("bossint_user_email", "saas-user@bossint.ai");
      if (pendingDeployDetails) {
        localStorage.setItem("bossint_pending_deploy", JSON.stringify(pendingDeployDetails));
      }
    ...
    if (success) {
      ...
      if ((modalEmail.trim().toLowerCase() === "gokhan@bossint.ai" || modalEmail.trim().toLowerCase() === "gokhan@gunery.com") && modalPassword === "112233q") {
        localStorage.setItem("bossint_auth", "true");
        localStorage.setItem("bossint_user_token", "sk_1f6a88dd6e054a399e971163a1e15c6b");
        localStorage.setItem("bossint_user_email", "gokhan@bossint.ai");
        if (pendingDeployDetails) {
        ...
        setModalError("Invalid credentials. Try: gokhan@bossint.ai / 112233q");
      }
    }
    setIsLoading(false);
  };
